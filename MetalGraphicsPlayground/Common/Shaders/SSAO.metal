...
kernel void ssao(texture2d<float> normal [[texture(0)]],
    ...
    float3 n = normalize(normal.read(thread_pos * downscale).xyz * 2.0 - 1.0);
    float3 t = normalize(tangent.read(thread_pos * downscale).xyz * 2.0 - 1.0);
    float3 b = cross(t, n);
    
    float3 world_pos = pos.read(thread_pos * downscale).xyz;
    ...
    for(uint i = 0; i < num_samples; i++) {
        ...
        float3 world_sample_pos = pos.read(screen_offset_pos * downscale).xyz;
